...
app.get('/status', (req, res) => {
  ...
  try {
    const df = require('child_process').execSync('df -h / | tail -1').toString().trim().split(/\s+/);
    diskInfo.total = df[1];
    diskInfo.used = df[2];
...
app.get('/torrentfile', (req, res) => {
  ...
  try {
    const httpLib = torrent.startsWith('https') ? require('https') : require('http');
    ...
    console.error('Error processing torrent URL request:', error);
  }
});
