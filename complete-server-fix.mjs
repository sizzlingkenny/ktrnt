...
let cleanupProcess;
try {
  cleanupProcess = spawn('node', ['cleanup.mjs'], { 
   detached: true,
   stdio: 'inherit'
  }); 
...
app.get('/torrents', (req, res) => {
  ...
  if (req.query.json === 'true') {
	...
	app.get('/magnet', (req, res) => {
	  const { uri } = req.query;
	  const wantJson = req.query.json === 'true' || req.headers.accept.includes('application/json');
	  ...
	  if (existingTorrent) {
		...
		return sendTorrentResponse(existingTorrent, res, wantJson); // ✅ FIXED
	  }

// Add new torrent by magnet URI
const torrent = client.add(uri, { path: DOWNLOAD_PATH });

torrent.on('metadata', () => {
  console.log('Metadata retrieved:', torrent.name);
  updateTorrentActivity(torrent.infoHash);
  sendTorrentResponse(torrent, res, wantJson); // Respond when metadata is ready
});
...
// Immediate response to client that torrent is processing
if (wantJson) {
  res.json({
    status: 'processing',
    infoHash: infoHash,
...
const torrentOptions = { 
  path: downloadsPath,
  maxWebConns: 10, // Limit web connections to avoid overload
};
...
client.add(uri, torrentOptions, (torrent) => {
  ...
  // Prioritize first 10% and last 10% pieces for streaming efficiency
...
// Function to send torrent details response
function sendTorrentResponse(torrent, res, wantJson) {
  if (!torrent || !torrent.files) {
    console.error('Torrent or torrent.files is undefined or not ready');
    return res.status(400).json({ error: 'Torrent files not found or metadata not ready yet.' });
  }
...
// Endpoint to add torrent by URL
app.get('/torrentfile', (req, res) => {
  ...
  // Background download and add torrent
  try {
    ...
    httpLib.get(torrent, options, (response) => {
      if ([301, 302].includes(response.statusCode)) {
        ...
        // Optionally follow redirects here
        return;
      }
      ...
      response.on('end', () => {
        ...
        // Save temp file for adding torrent
...
// Upload a torrent file endpoint
app.post('/upload', upload.single('torrent'), (req, res) => {
  ...
  // ... continue with adding torrent from uploaded file ...
});
...
app.get('/remove/:infoHash', (req, res) => {
  ...
  try {
    ...
    if (req.query.json === 'true') {
      res.json({
        success: true,
        message: `Removed torrent: ${torrentName} (${infoHash})`
      });
    } else {
      res.send(`Removed torrent: ${torrentName} (${infoHash})`);
    }
  } catch (err) {
    console.error(`Error removing torrent ${infoHash}:`, err);
    res.status(500).send(`Failed to remove torrent: ${err.message}`);
  }
});
